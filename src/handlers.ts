...
export const generate = async () => {
  try {
    ...
    parent.postMessage({ resp: 'generate', error: true, message: e.message }, '*');
  }
};
...
export const sign = async (publicKey: string) => {
  try {
    ...
    parent.postMessage({ resp: 'sign', error: true, message: e.message }, '*');
  }
};
...
export const download = () => {
  try {
    ...
    parent.postMessage({ resp: 'download', error: true, message: e.message }, '*');
  }
};
